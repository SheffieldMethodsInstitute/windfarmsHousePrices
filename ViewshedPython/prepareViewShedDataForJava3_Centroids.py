...
#This version: for postcode centroids and windfarm centroids (and two versions of windfarm centroids...)
...
def run_script(iface):
	...
	#Whole windfarms
	# uri = "file:///C:/Data/WindFarmViewShed/ViewshedPython/Data/whole_windfarm_centroids.csv?type=csv&xField=Feature.Easting&yField=Feature.Northing&spatialIndex=no&subsetIndex=no&watchFile=no&crs=EPSG:27700"
	#Windfarms broken down by extension (if they have multiple extensions...)
	#Also changed folder to centroids2 for this... hackery...
	uri = "file:///C:/Data/WindFarmViewShed/ViewshedPython/Data/windfarm_centroids_extensions_separate.csv?type=csv&xField=Feature.Easting&yField=Feature.Northing&spatialIndex=no&subsetIndex=no&watchFile=no&crs=EPSG:27700"
	...
	#Load from CSV
	uri = "file:///C:/Data/WindFarmViewShed/ViewshedPython/Data/postcode_centroids.csv?type=csv&xField=x&yField=y&spatialIndex=no&subsetIndex=no&watchFile=no&crs=EPSG:27700"
	...
	geometryanalyzer.buffer(turbines, 
		'C:/Data/WindFarmViewShed/Tests/PythonTests/testData/SinglePart_2point75kmBuffers_centroids2.shp', 
		2750, False, True, -1)
	...
	for buffr in buffLyr.getFeatures():
		...
		filename = ('ViewShedJava/SimpleViewShed/data_centroids2/observers/' + str(buffr.id()) + '.csv')
		...
		'ViewShedJava/SimpleViewShed/data/temp/15kmBuffer_centroids2' + str (buffr.id()) + '.shp', 
		15000, False, True, -1)
		...
		filename = ('ViewShedJava/SimpleViewShed/data_centroids2/targets/' + str(buffr.id()) + '.csv')
		...
		filenametif1 = ('ViewShedJava/SimpleViewShed/data_centroids2/rasters/' +
			str(buffr.id()) + 
			'.tif')
		...
		#Newp: no building heights for centroids thx plz
		# before = time.time()

		# filenametif2 = ('ViewShedJava/SimpleViewShed/data_centroids1/rasters_w_buildingheight/' +
		# 	str(buffr.id()) + 
		# 	'.tif')
		...
		# write in own file
		text_file = open(
			"C:/Data/WindFarmViewShed/ViewShedJava/SimpleViewShed/data_centroids2/coords/" + str(buffr.id()) + ".txt",
			"w")
